...
<template>
  <div>
    <SubTitle v-if="title">
      {{ resolvedTitle }}
      <NumberBadge v-if="list.length" :number="list.length" class="rounded-full text-sm" />
      <button
        v-if="reversable"
        title="Reverse"
    ...
    <div class="relative overflow-hidden panel-card" :class="containerClass">
      ...
      >
        ...
        >
          ...
          <span>More</span>
          <NumberBadge prefix="+" :number="Math.min(Math.round(count * 0.5), props.list.length)" class="rounded-full text-sm" />
        </button>
        <button
          class="op35 p-2 pt-4 mt-auto pointer-events-auto hover:op100 flex items-center gap-1 justify-center"
        ...
        >
          ...
          <span>All</span>
          <NumberBadge :number="props.list.length" class="rounded-full text-sm" />
        </button>
      </div>
    </div>
  </div>
