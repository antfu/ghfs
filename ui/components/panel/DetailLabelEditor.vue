...
<template>
  <UiModal
    v-model:open="ui.labelEditorOpen.value"
    icon="i-octicon-tag-16"
    width="w-[min(92vw,32rem)]"
    max-height="max-h-[70vh]"
    data-testid="label-editor"
    description="Toggle labels to queue a set-labels change for this item."
    @keydown="onListKeydown"
  >
    <template #header>
      <span class="i-octicon-tag-16 color-active shrink-0" />
      <span class="font-medium text-sm">Edit labels</span>
    ...
    <ul class="py-1">
      ...
      <li
        ...
        :key="label.name"
        class="flex items-center gap-3 px-3 py-1.5 text-sm cursor-pointer transition"
        :class="highlighted === idx ? 'bg-active' : 'hover:bg-active'"
        @mouseenter="highlighted = idx"
        @click="toggle(label.name)"
      >
        <span
          class="inline-flex items-center justify-center w-4 h-4 rounded border border-base shrink-0 transition"
          :class="selected.has(label.name) ? 'bg-primary-500 border-primary-500' : ''"
        >
          <span v-if="selected.has(label.name)" class="i-ph-check-bold text-white text-xs" />
        </span>
        <span
    ...
    <template #footer>
      ...
      <div class="flex-1" />
      <span class="text-xs color-muted">{{ selected.size }} selected</span>
    </template>
  </UiModal>
</template>
