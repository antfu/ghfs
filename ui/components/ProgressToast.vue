...
<template>
  ...
  >
    <div
      v-if="visible"
      class="fixed bottom-4 left-1/2 -translate-x-1/2 z-toast min-w-80 max-w-lg panel-card shadow-lg"
    >
      <div v-if="state.lastError.value" class="flex items-start gap-3 px-4 py-3">
        <span class="i-ph-warning-duotone color-red-500 mt-0.5 flex-none" />
      ...
      <div v-else-if="state.progress.value" class="flex items-start gap-3 px-4 py-3">
        ...
        <div class="flex-1">
          ...
          <div v-if="state.progress.value.percent != null" class="w-full h-1 bg-#8882 rounded mt-2 overflow-hidden">
            <div
