...
import { DETAILS_MD_FILE_NAME, EXECUTE_MD_FILE_NAME, EXECUTE_SCHEMA_RELATIVE_PATH } from '../constants'
...
export const DETAILS_MD_FILE_PLACEHOLDER = [
  '# Placeholder for future details-md execution source.',
  '',
].join('\n')
...
export async function ensureExecuteArtifacts(executeFilePath: string): Promise<{
  ...
  const [schemaPath] = await Promise.all([
    ...
    ensureDetailsMdFile(storageDirAbsolute),
  ])
...
async function ensureDetailsMdFile(storageDirAbsolute: string): Promise<void> {
  const detailsMdPath = join(storageDirAbsolute, DETAILS_MD_FILE_NAME)
  if (await pathExists(detailsMdPath))
    return

  await mkdir(dirname(detailsMdPath), { recursive: true })
  await writeFile(detailsMdPath, DETAILS_MD_FILE_PLACEHOLDER, 'utf8')
}
